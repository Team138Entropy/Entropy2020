    ...
    private class EnabledLoop implements Loop {
        ...
        public synchronized void onLoop(double timestamp) {
            ...
            final Twist2d measured_velocity = Kinematics.forwardKinematics(
                    delta_left, delta_right, mPrev_Rotation.inverse().rotateBy(gyro_angle).getRadians()).scaled(1.0 / dt);
           ...
           // mRobotTracker.addVehicleToTurretObservation(timestamp, Rotation2d.fromDegrees(Turret.getInstance().getAngle()));
            mRobotTracker.addDriveObservations(timestamp, odometry_twist, measured_velocity, predicted_velocity);
