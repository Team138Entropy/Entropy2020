...
import frc.robot.util.motion.MotionProfileGoal.CompletionBehavior;
...
public class ProfileFollower {
     ...
     * @param kffv The feedforward gain on velocity. Should be 1.0 if the units of the profile match
     ...
     */
    public ProfileFollower(double kp, double ki, double kv, double kffv, double kffa, double ks) {
        resetProfile();
    ...
    public void setGoalAndConstraints(
            MotionProfileGoal goal, MotionProfileConstraints constraints) {
     ...
     * rather than feeding forward the previous setpoint the next time update() is called. This
     * almost always forces a MotionProfile update, and may be warranted if tracking error gets very
     * large.
     */
     ...
     *
     * @param latest_state The latest *actual* state, used only for feedback purposes (unless this
     *     is the first iteration or reset()/resetSetpoint() was just called, in which case this is
     *     the new start state for the profile).
    ...
    public synchronized double update(MotionState latest_state, double t) {
        ...
        double output =
                mKp * mLatestPosError
                        + mKv * mLatestVelError
                        + mKffv * mLatestSetpoint.motion_state.vel()
                        + (Double.isNaN(mLatestSetpoint.motion_state.acc())
                                ? 0.0
                                : mKffa * mLatestSetpoint.motion_state.acc());
    ...
    /**
     * We are finished the profile when the final setpoint has been generated. Note that this does
     * not check whether we are anywhere close to the final setpoint, however.
     *
    ...
    public boolean onTarget() {
        ...
        // For the options that don't achieve the goal velocity exactly, also count any instance
        ...
        final boolean passed_goal_state =
                Math.signum(goal_to_start) * Math.signum(goal_to_actual) < 0.0;
        return mGoal.atGoalState(mLatestActualState)
                || (mGoal.completion_behavior() != CompletionBehavior.OVERSHOOT
                        && passed_goal_state);
    }
}
