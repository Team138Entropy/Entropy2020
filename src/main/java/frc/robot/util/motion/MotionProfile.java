...
public class MotionProfile {
    ...
    /**
     * Get the interpolated MotionState at any given time, clamping to the endpoints if time is out
     * of bounds.
     *
     * @param t The time to query.
    ...
    public void consolidate() {
        for (Iterator<MotionSegment> iterator = mSegments.iterator();
                iterator.hasNext() && mSegments.size() > 1; ) {
            MotionSegment s = iterator.next();
    ...
    public void appendControl(double acc, double dt) {
        ...
        MotionState new_start_state =
                new MotionState(
                        last_end_state.t(), last_end_state.pos(), last_end_state.vel(), acc);
        appendSegment(
                new MotionSegment(
                        new_start_state, new_start_state.extrapolate(new_start_state.t() + dt)));
    }
