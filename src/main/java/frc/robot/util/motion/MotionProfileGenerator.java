...
public class MotionProfileGenerator {
    ...
    protected static MotionProfile generateFlippedProfile(
        ...
        MotionProfile profile =
                generateProfile(constraints, goal_state.flipped(), prev_state.flipped());
    ...
    public static synchronized MotionProfile generateProfile(
        ...
        MotionState start_state =
                new MotionState(
                        prev_state.t(),
                        prev_state.pos(),
                        Math.signum(prev_state.vel())
                                * Math.min(Math.abs(prev_state.vel()), constraints.max_abs_vel()),
                        Math.signum(prev_state.acc())
                                * Math.min(Math.abs(prev_state.acc()), constraints.max_abs_acc()));
        ...
        final double min_abs_vel_at_goal_sqr =
                start_state.vel2() - 2.0 * constraints.max_abs_acc() * delta_pos;
        final double min_abs_vel_at_goal = Math.sqrt(Math.abs(min_abs_vel_at_goal_sqr));
        final double max_abs_vel_at_goal =
                Math.sqrt(start_state.vel2() + 2.0 * constraints.max_abs_acc() * delta_pos);
        ...
        if (min_abs_vel_at_goal_sqr > 0.0
            ...
            // Overshoot is unavoidable with the current constraints. Look at completion_behavior to
            // see what we should
            // do.
            if (goal_state.completion_behavior() == CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
                ...
                // We are going to overshoot the goal, so the first thing we need to do is come to a
                // stop.
                final double stopping_time =
                        Math.abs(start_state.vel() / constraints.max_abs_acc());
                profile.appendControl(-constraints.max_abs_acc(), stopping_time);
                // Now we need to travel backwards, so generate a flipped profile.
                profile.appendProfile(
                        generateFlippedProfile(constraints, goal_state, profile.endState()));
                profile.consolidate();
                return profile;
            }
        }
        goal_vel = Math.min(goal_vel, max_abs_vel_at_goal);
        // Invariant from this point forward: We can achieve goal_vel at goal_state.pos exactly
        // using no more than +/-
        // max_acc.

        // What is the maximum velocity we can reach (Vmax)? This is the intersection of two curves:
        // one accelerating
        ...
        final double v_max =
                Math.min(
                        constraints.max_abs_vel(),
                        Math.sqrt(
                                (start_state.vel2() + goal_vel * goal_vel) / 2.0
                                        + delta_pos * max_acc));
        ...
        final double distance_decel =
                Math.max(
                        0.0,
                        (start_state.vel2() - goal_vel * goal_vel)
                                / (2.0 * constraints.max_abs_acc()));
        final double distance_cruise =
                Math.max(0.0, goal_state.pos() - start_state.pos() - distance_decel);
